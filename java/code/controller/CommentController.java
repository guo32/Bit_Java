...
public class CommentController {
    ...
    public void updateComment(ArrayList<UserDTO> userList) {
        CommentDTO temp;
        for (CommentDTO c : list) {
            if (userList.indexOf(new UserDTO(c.getUserId())) == -1) {
                temp = new CommentDTO(c);
                temp.setUserNickname("탈퇴한 사용자");
                update(temp);
            } else {
                UserDTO testUser = userList.get(userList.indexOf(new UserDTO(c.getUserId())));
                if(!testUser.getNickname().equals(c.getUserNickname())) {
                    temp = new CommentDTO(c);
                    temp.setUserNickname(testUser.getNickname());
                    update(temp);
                }
            }
        }
